...
static const DemoPose DEMO_BENCHMARKS[DEMO_POSE_COUNT] = {
    /* 0: Pushup Bottom Inflection */
    {
        ...
        .target_joint_a = { .x = 0.38, .y = 0.50, .confidence = 1.0f, .is_valid = true }, /* Shoulder */
        .target_joint_b = { .x = 0.38, .y = 0.65, .confidence = 1.0f, .is_valid = true }, /* Elbow */
        .target_joint_c = { .x = 0.38, .y = 0.80, .confidence = 1.0f, .is_valid = true }, /* Wrist */
        .target_spine_ref = { .x = 0.70, .y = 0.50, .confidence = 1.0f, .is_valid = true }, /* Hip */
    ...
    /* 2: Squat Parallel Depth */
    {
        ...
        .angle_tolerance_deg = 10.0,
        .target_joint_a = { .x = 0.42, .y = 0.52, .confidence = 1.0f, .is_valid = true }, /* Hip */
        ...
        .target_spine_ref = { .x = 0.40, .y = 0.28, .confidence = 1.0f, .is_valid = true }, /* Shoulder */
    ...
    /* 4: Plank Isometric Core Hold */
    {
        ...
        .target_joint_a = { .x = 0.30, .y = 0.55, .confidence = 1.0f, .is_valid = true }, /* Shoulder */
        .target_joint_b = { .x = 0.30, .y = 0.72, .confidence = 1.0f, .is_valid = true }, /* Elbow */
        .target_joint_c = { .x = 0.45, .y = 0.72, .confidence = 1.0f, .is_valid = true }, /* Wrist/Hand */
        .target_spine_ref = { .x = 0.60, .y = 0.55, .confidence = 1.0f, .is_valid = true }, /* Hip */
...
const DemoPose* demo_pose_get_by_exercise(const char* exercise_name, bool is_inflection_phase) {
    ...
    if (strstr(exercise_name, "Pushup") || strstr(exercise_name, "pushup")) {
...
void demo_pose_evaluate_form(const DemoPose* demo,
    ...
    if (user_spine_ref.is_valid && demo->target_secondary_angle_deg > 0.0) {
        ...
        /* Calculate secondary angle: joint_a -> spine_ref -> joint_b (e.g. shoulder -> hip -> knee) */
